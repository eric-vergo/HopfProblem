...
which the blueprint site quotes as dated verifier evidence. The artifact
therefore crosses a trust boundary, and codex-audit CX-052 showed that a
...
Ported from eric-vergo/OEIS-A362583-Irrationality `scripts/validate_comparator_result.py`.
Two things differ, both because this repository's layout differs:

  * the certified sources are `Challenge.lean` and `Solution.lean` at the
    REPOSITORY ROOT (a362583 keeps them under `comparator/`), which is a
    `--hash-file` argument here rather than anything this file hard-codes; and
  * the record carries a `kernel_identities` array -- the run's own record of
    WHICH nanoda executable replayed the export (codex-audit CX-064). It is
    validated against the manifest digest of the same binary, so the identity
    and the hash manifest cannot disagree about the program that ran.
