#!/usr/bin/env python3
"""Forged-build fixtures for scripts/check_site_release.py.

The Pages deploy workflow publishes a site nobody in CI generated. What stands
between "an operator uploaded a tarball" and "master vouches for these bytes" is
this gate, and a gate is only as good as the trees it refuses. Each fixture here
is a site that a hash-only check would have published, and each test names the
property it must be caught by:
...
Run: python3 scripts/tests/test_check_site_release.py
"""
...
class Fixture:
    ...
    def package(self, revision=None, forge_sha=None):
        revision = revision or self.revision
        ...
        pin = {
            ...
            "pins": {"toolchain": "leanprover/lean4:v4.33.0"},
        }
