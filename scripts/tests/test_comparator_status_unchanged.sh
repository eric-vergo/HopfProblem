#!/usr/bin/env bash
# Fixtures for scripts/comparator_status_unchanged.sh.
#
...
#
# Run: bash scripts/tests/test_comparator_status_unchanged.sh

set -uo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO="$(cd "${HERE}/../.." && pwd)"
SCRIPT="${REPO}/scripts/comparator_status_unchanged.sh"
