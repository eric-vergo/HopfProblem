...
#
# The publish job (.github/workflows/ci.yml) composes a status record from every
# run's evidence. Rewriting the committed one unconditionally would stamp a new
# `commit`, `verified_at` and `run_url` into the file on every push -- including
# pushes that touched nothing the comparator looked at. That file is an
# ELABORATION-TIME trust input of the blueprint site under site/: its freshness
# edge is an `input_file` dependency, so every rewrite invalidates the site's
# warm build and costs a ~3 h regeneration for a verdict that did not move.
#
...
# ones -- `commit`, `verified_at`, `run_url`, `note` -- which is the point:
...
#
#   usage: bash scripts/comparator_status_unchanged.sh COMMITTED CANDIDATE
#
...
#
# Tested by scripts/tests/test_comparator_status_unchanged.sh, in the trusted
